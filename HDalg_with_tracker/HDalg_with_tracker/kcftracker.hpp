...
public:
	// Constructor
	KCFTracker(int tmpl_sz = 64, float pad_sz = 2.2, float scalestep=1.07);

	// Initialize tracker 
	void init(CRect &roi, CMat image);

	// Update position based on the new frame
	CRect update(CMat image,int &times,float &Goal);
...
private:
	...
	CMat _tmpl;
	 ...
	 int lowgoaltimeslevel0;
	 int lowgoaltimeslevel1;                 //低分的次数
	 int lowgoaltimeslevel2;
	 int lowgoaltimeslevel3;
};
#endif