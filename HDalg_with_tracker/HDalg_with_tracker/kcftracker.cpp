...
#include <assert.h>
#include <string.h>
#include "kcftracker.hpp"                           //以下五个头文件在源代码中以ifndef格式定义
#include "basetype.h"
#include "fhog.hpp"
...
//#include "opencv2/opencv.hpp"	//-! added for test   该定义为增加部分
#ifndef min                                            
#define min(a,b)            (((a) < (b)) ? (a) : (b))
#endif

#define MISSTARGET 15


void __resizeNN_gray(const unsigned char *srcData,          //该函数为增加部分，分析应该是修改某个区域灰度图像的尺寸
...
// Constructor
KCFTracker::KCFTracker(int tmpl_sz, float pad_sz, float scalestep)
{
...
void KCFTracker::init(CRect &roi, CMat image)
{
	...
	//printCMat2f(_prob);
	_alphaf = CMat(size_patch[0], size_patch[1], MAT_32FC2);          ////创建_alphaf矩阵数据和类型，但没有赋值
	memset(_alphaf.data,0,_alphaf.rows*_alphaf.step);	//init memory, may be wrong   对该矩阵进行赋值
	//printCMat2f(_alphaf);
	train(_tmpl, 1.0); // train with initial frame

	goal.clear();                                 //初始化清空，避免目标丢失后再次初始化存在该数据，影响初始化跟踪器的效率
...
#ifdef CKALMAN
	Object cklobject;                        ////added by gelin 2017-9-7
	cklobject.x=roi.x;
	cklobject.y=roi.y;
	cklobject.vx=0;
	cklobject.vy=0;
	ckalman.Init(cklobject);                ////end added
	//printf("ckalman is on!\n");
#endif
...
CRect KCFTracker::update(CMat image,int &times,float &Goal)
{
#ifdef KALMAN
...
#endif 
#ifdef CKALMAN
	ckalman.Predict(4,2);                                 ////对roi的x y位置进行预测更正  added by gelin 2017-9-7
	Object cklobject=ckalman.GetObject();
	_roi.x=cklobject.x;
	_roi.y=cklobject.y;                          
#endif 
	//printf("kalman:x %f y%f ckalman: x %f y %f\n",klobject.x,klobject.y,cklobject.x,cklobject.y);
	...
	CPoint2f res = detect(_tmpl, map1, peak_value);
	Choosesize=1;
	...
	//	// Test at a smaller _scale
		...
		CPoint2f new_res = detect(_tmpl, map2, new_peak_value);
...
#endif
#ifdef CKALMAN
	KLRect updatecklobj;                                          ////added by gelin 2017-8-24
	updatecklobj.x=_roi.x;
	updatecklobj.y=_roi.y;    
	ckalman.Update(updatecklobj,4,2);                  
	cklobject=ckalman.GetObject();
	_roi.x=cklobject.x;
	_roi.y=cklobject.y;
#endif 
	//printf("kalman:x %f y%f ckalman: x %f y %f\n",klobject.x,klobject.y,cklobject.x,cklobject.y);
	...
	Goal=peak_value;
	if(goal.size()<=20) goal.push_back(peak_value);
	else{
		goal.erase(goal.begin());
		goal.push_back(peak_value);
	}
	lowgoaltimeslevel0=0; lowgoaltimeslevel1=0; lowgoaltimeslevel2=0;lowgoaltimeslevel3=0;
	for(std::vector<float>::iterator it=goal.begin();it!=goal.end();it++)
	{
		if((*it)<0.12) lowgoaltimeslevel0++,lowgoaltimeslevel1++,lowgoaltimeslevel2++,lowgoaltimeslevel3++;
		else if((*it)<0.15) lowgoaltimeslevel0=0,lowgoaltimeslevel1++,lowgoaltimeslevel2++,lowgoaltimeslevel3++;
		else if((*it)<0.2) lowgoaltimeslevel0=0,lowgoaltimeslevel1=0,lowgoaltimeslevel2++,lowgoaltimeslevel3++;
		else if((*it)<0.22) lowgoaltimeslevel0=0,lowgoaltimeslevel1=0,lowgoaltimeslevel2=0,lowgoaltimeslevel3++;
		else lowgoaltimeslevel0=0,lowgoaltimeslevel1=0,lowgoaltimeslevel2=0,lowgoaltimeslevel3=0;
	}
	if(lowgoaltimeslevel0>=5) times=MISSTARGET;
	else if(lowgoaltimeslevel1>=10) times=MISSTARGET;
	else if(lowgoaltimeslevel2>=15) times=MISSTARGET;
	else if(lowgoaltimeslevel3>=20) times=MISSTARGET;
	else times=lowgoaltimeslevel2;
	//printf("peakvalue:%f,missing times :level0: %d level1: %d level2: %d level3: %d!\n",peak_value,lowgoaltimeslevel0,lowgoaltimeslevel1,times,lowgoaltimeslevel3);
	//double time_profile_counter = (double)cvGetTickCount();
