...
/************************************************************************/
/* 快速遍历前景图像，输出bloblist          */  //分析最新一帧的前景图像中的块blob（包括分析大小、合并、裁剪），然后将分析的块存入bloblist中，然后对bloblist进行相邻块合并
/* param: src 输入的前景图像          */
...
int fastFindContourRects(const uchar *src,int width,int height,int widthStride,vector<HdRect> &blobList, HdSize operateSz, double ratio)
{
	...
	//printf("*************************BlobList size :%d\n",blobList.size()); //传入的bloblist都是空的
	...
	float rate=0.3;                                                  //该参数取0.25-0.35之间鲁棒性比较好
	float uprate=0.15;
...
int fastFindContourRects2(const uchar *src,int width,int height,int widthStride,vector<HdRect> &blobList, HdSize operateSz, double ratio)
{
	...
	float covermid=0.5,coverhigh=0.75,coverlow=0.25;
	...
	for(vector<HdRect>::iterator sq=seq.begin();sq!=seq.end();sq++)
	{
		for(int j=sq->x;j<=sq->x+sq->width-1;j++)
		{
			...
			if(j+1==sq->x+sq->width){
				...
				divid=true;
			}else if((countsame*1.0/posifg1>covermid&&countsame*1.0/posifg2>covermid)||
				(countsame*1.0/posifg1>coverhigh&&countsame*1.0/posifg2>coverlow)||
				(countsame*1.0/posifg1>coverlow&&countsame*1.0/posifg2>coverhigh))
			{
				endcol=j+1;
				divid=false;
			} else {
	...
	for(vector<HdRect>::iterator sq=seqfinal.begin();sq!=seqfinal.end();sq++)    //去掉内部前景点很少的矩形
	{
		...
		if(noempty*1.0/(sq->width*sq->height)>noemptyratelow) seq.push_back(*sq);
		noempty=0;
	}
	seqfinal.clear();
	...
	int treshod=0;
	int goal=0;  int sum=0;  int sumcount=0;
	float rate=0.35;                                                  //该参数取0.25-0.35之间鲁棒性比较好
	float uprate=0.35;
