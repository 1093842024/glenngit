...
VibeParam* CreateBgParam()
{
    ...
    ptr->w = ptr->h = 0;                 //模型宽高
    ptr->depth = 14;                     //模型大小  默认16     每个样本的背景模型样本数量
    ptr->blockD = 5;                     //邻域？大小 默认8？9   初始化时在某个邻域内随机去值作为背景模型
    ptr->nTh = 2;                        //个数阈值  默认2
    ptr->Radius = 15;                    //距离半径  默认20
    ptr->pModel = NULL;
    ptr->pFgImg = NULL;
    ...
    ptr->alarmPercentage = 0.001;         //前景点数达到这个比例会触发报警，灵敏度参数之一
...
void setSens(VibeParam* ptr, unsigned int m_uMDSensitive)
{
    if (ptr->Sensitive != m_uMDSensitive)
    {
        ...
        case 0:
            ptr->Radius = 15;
            ptr->alarmPercentage = 0.001;  // 前景比例尽量小
...
int UpdateBgModel( const uchar* ptr_gray, const int width, const int height, const int step, unsigned int m_uMDSensitive, VibeParam *phd/*Handle*/ )
{
	...
	if ((phd->justInitial && totalCnt > 0.2*wholeArea) || totalCnt > 0.4*wholeArea)
	{
