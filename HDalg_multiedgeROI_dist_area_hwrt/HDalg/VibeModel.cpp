...
VibeParam* CreateBgParam()
{
    ...
    ptr->pModel = NULL;
    ptr->pFgImg = NULL;
    ptr->pHistoryImage = NULL;
    ptr->lastHistoryImageSwapped = 0;
    ptr->frameCnt = 0;
    ptr->jump = NULL;
    ...
    ptr->alarmPercentage = 0.0005;
    ptr->justInitial = false;
    ptr->Sensitive = 0;

    ptr->roi_x = ptr->roi_y = ptr->roi_w = ptr->roi_h = -1;
...
#endif
        ptr->Sensitive = m_uMDSensitive;
        switch (ptr->Sensitive)     // 灵敏度设置，分0-6共7个档，灵敏度依次降低。
        {
        case 0:
            ptr->Radius = 5;
...
bool InitBackModel( const uchar* ptr_gray, const int width, const int height, const int step, unsigned int m_uMDSensitive, VibeParam* phd )
{
	...
	phd->AeraR=phd->roi_w*phd->roi_h*1.0/(width*height);
    ...
    phd->pModel = new uchar[phd->w*phd->h*phd->depth];
    phd->pHistoryImage = new uchar[phd->w*phd->h*2];
    phd->pFgImg = new uchar[phd->w*phd->h];
    phd->prevImg = new uchar[phd->w*phd->h];
    phd->cntImg  = new uchar[phd->w*phd->h];
    phd->jump = new int[2*phd->w+1];
    memset(phd->cntImg,0,sizeof(uchar)*phd->w*phd->h);
    phd->alarmCntTh = MAX(MIN_FG_PTS,int(phd->w*phd->h*phd->alarmPercentage));
                                                                 //////该值决定了能够找到的最小的区域变化情况
...
int UpdateBgModel( const uchar* ptr_gray, const int width, const int height, const int step, unsigned int m_uMDSensitive, VibeParam *phd/*Handle*/ )
{
	...
	{
		phd->frameCnt ++;
		InitBackModel( ptr_gray, width, height, step, m_uMDSensitive,phd );
		return INIT_MODULE;
	}

    memset( phd->pFgImg, 0, sizeof(uchar)*phd->w*phd->h);
    // Calculate the foreGround and update model
    int blockR = phd->blockD/2;
    int mWS = phd->w*phd->depth, gWS = step, depth = phd->depth;
    //int blockSize = 2*phd->blockR + 1;
    int blockStep = gWS*phd->blockD;
    const uchar* pG = ptr_gray + (phd->roi_y+blockR+phd->blockD*(phd->h-1))*gWS + phd->roi_x + blockR ;    //从所选区域的最下面一行开始
	...
	if ((phd->justInitial && totalCnt > 0.1*wholeArea) || totalCnt > 0.4*wholeArea)         //这个0.1商榷有待
	{
