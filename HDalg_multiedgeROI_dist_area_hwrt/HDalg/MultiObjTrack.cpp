...
bool InitObjList(MatchParam *pMP, ObjList *objList, vector<HdRect> &blobList)
{
	...
	{	
		...
		if (needNewObj && objList->len < MAX_OBJ_NUM)     ////新的bolb满足上面的距离条件，并且objlist中的obj的数量不超过50个，则新建obj。
		{
 ...
 /*           _ObjRect rect;                                        ////将该目标的矩形框信息传到这个目标自己的矩形框记录队列中 added by gelin 2017-8-23
			rect.w= tmp->width;
			rect.h= tmp->height;
			rect.x= tmp->x;
			rect.y= tmp->y;
			time_t tim=time(NULL);
			if(objNode->obj.pts.size()<MAX_RECT_NUM)              
			{
			      objNode->obj.pts.push_back(rect);
				  objNode->obj.pts_time.push_back(tim);
			}else{
			      objNode->obj.pts.pop_front();
				  objNode->obj.pts.push_back(rect);
				  objNode->obj.pts_time.pop_front();
				  objNode->obj.pts_time.push_back(tim);
			}                                                     ////修改结束1*/

#ifdef HISTORY_PARAM                                              ////记录该目标最近几（5）帧的位置信息
            objNode->obj.idx = 0;
...
/*				_ObjRect rect;                                        ////将该目标的矩形框信息传到这个目标自己的矩形框记录队列中 added by gelin 2017-8-23
			    rect.w= blobList.at(i).width;
			    rect.h= blobList.at(i).height;
			    rect.x= blobList.at(i).x;
			    rect.y= blobList.at(i).y;
				time_t tim=time(NULL);
				if(objNode->obj.pts.size()<MAX_RECT_NUM)              
				{
					objNode->obj.pts.push_back(rect);
...
//				pNext->obj.h = (pNext->obj.h*ts + blobList->at(bestIndex).height)/(ts+1);
				...
				_ObjRect rect;                                        ////将该目标的矩形框信息传到这个目标自己的矩形框记录队列中 added by gelin 2017-8-23
			    rect.w= blobList.at(bestIndex).width;
			    rect.h= blobList.at(bestIndex).height;
			    rect.x= blobList.at(bestIndex).x;
			    rect.y= blobList.at(bestIndex).y;


				time_t tim=time(NULL);
				if(pNext->obj.pts.size()<MAX_RECT_NUM)              
				{
					pNext->obj.pts.push_back(rect);
					pNext->obj.pts_time.push_back(tim);
				}else{
					pNext->obj.pts.pop_front();
					pNext->obj.pts.push_back(rect);
					pNext->obj.pts_time.pop_front();
					pNext->obj.pts_time.push_back(tim);
				}  

				std::list<time_t>::iterator i=pNext->obj.pts_time.begin();
	            int num=0;
				for(;i!=pNext->obj.pts_time.end();i++)   //由于遍历过程删除容器中的元素会导致迭代器指针失效，因此不能使用该条件的时候删除元素
				{
				    if(tim-*i>3) num++;	
				}
				for(int k=0;k<num;k++)
				...
				/*if(pNext->obj.pts.size()>0)
				{
				    _ObjRect rectlast= pNext->obj.pts.back();
					if((rectlast.x-rect.x)*(rectlast.x-rect.x)<400 &&(rectlast.y-rect.y)*(rectlast.y-rect.y)<400)
					{
						time_t tim=time(NULL);
						if(pNext->obj.pts.size()<MAX_RECT_NUM)              
						{
