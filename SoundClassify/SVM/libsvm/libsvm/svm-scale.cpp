...
int SVM_SCALE(int argc,char **argv)
{
	int i,index;
	FILE *fp, *fp_restore = NULL;
	char *save_filename = NULL;
	char *restore_filename = NULL;
	char *scaled_filename=NULL;
	...
	char name[100];               //创建保存scale过的文件名称
	strcpy(name,argv[i]);
	char *nameafter=".scale";
	strcat(name,nameafter);
	scaled_filename=name;
...
#define SKIP_ELEMENT\
	...
	/* pass 3: scale */            //根据找到的范围，进行缩放，然后打印再屏幕上
	fp_scaled = fopen(scaled_filename,"w");
	printf("starting to scale features,waiting.....\n");
	while(readline_scale(fp)!=NULL)
	{
		char *p=line;
		int next_index=1;
		...
		if (sscanf(p,"%lf",&target) != 1)    //读取特征向量的类别值
			return clean_up(NULL, fp, "ERROR: failed to read labels\n");
		output_target(target);

		SKIP_TARGET

		while(sscanf(p,"%d:%lf",&index,&value)==2)   //读取每个特征索引和特征值
		{
			for(i=next_index;i<index;i++)    //省略没有的特征索引
				output(i,0);
			
			output(index,value);        //输出特征索引和特征值
...
#endif
		fprintf(fp_scaled,"%d:%g ",index,value);
		new_num_nonzeros++;
	}
}
