...
void SoundClassify::InitGMM_Models(int Model_num)
{
	...
	for(int i=0;i<Model_num;i++)
	{
	    if(i==0)
		{
			...
			gmmmodel.MaxProb=-22;
			gmmmodel.AverAllProb=-22;
			gmmmodel.Maxprobstrict=-20;
			gmmmodel.AverAllProbstrict=-20;
...
int SoundClassify::ValideSoundClassify_Valide(const char *wavdata,int len, int soundtype)
{
	if(len%2!=0||len<512) {printf("sample dots nums are odd\n");return 0;}
	//载入音频，提取各种特征参数
	Wave *w = filter_bank(wavdata,soundtype,len);
	...
	for(;gmmnow!=gmmend;gmmnow++)
	{
			...
			for (int i = 0; i < MFCCCOEF*w->nRow; i++)   //输出了
			{
				...
				}else 
				{
					if(i%MFCCCOEF==0)
					{
						for(int j=i;j<i+MFCCCOEF;j++)
						{
							alldata[i / MFCCCOEF][j%MFCCCOEF]=0;  	
						}
					}
					alldata[i / MFCCCOEF][i%MFCCCOEF+MFCCCOEF]=0;
				}
			}
		}else 
		{
		...
		if(proball*FRAMENUM/(validenum+1)>1E-50) proball=log10(proball*FRAMENUM/(validenum+1));
		else proball=-50;
...
int SoundClassify::ValideSoundClassify(const string soundpath,vector<string> &classname,int soundtype)  //default soundtype=0 means "*.wav" audio
{
    ...
    for(;gmmnow!=gmmend;gmmnow++)
	{
		...
		gmmnow->sampleMax=probmax;
		gmmnow->sampleAverall=proball;
		printf("%.2f,%.2f\n",gmmnow->sampleAverall,gmmnow->sampleMax);
		delete singledata;
		for(int i=0; i<w->nRow;i++) 
			delete []alldata[i];
		delete alldata;
...
void SoundClassify::ChangeGMMgoal(int Gmmnum,int maxgoal,int avergoal)
{
	for(vector<GMM_Model>::iterator it=Gmm_model.begin();it!=Gmm_model.end();it++)
	{
		if(it->GMM_Num==Gmmnum)
		{
			it->AverAllProb=maxgoal;
			it->MaxProb=avergoal;
			printf("GMM********************change maxgoal and aver goal complete!\n");
		}
	}
}

void SoundClassify::ChangeGMMgoalStrict(int Gmmnum,int maxgoal,int avergoal)
{
	for(vector<GMM_Model>::iterator it=Gmm_model.begin();it!=Gmm_model.end();it++)
	{
		if(it->GMM_Num==Gmmnum)
		{
			it->AverAllProbstrict=maxgoal;
			it->Maxprobstrict=avergoal;
			printf("GMM********************change maxgoal and aver Strict goal complete!\n");
		}
	}
}