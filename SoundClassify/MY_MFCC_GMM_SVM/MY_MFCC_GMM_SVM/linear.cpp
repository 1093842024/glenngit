...
{
public:
	static double nrm2_sq(const feature_node *x)   //求一个样本向量的值平方的和
	{
		double ret = 0;
		while(x->index != -1)
		{
	...
	static double dot(const double *s, const feature_node *x)    //计算一个样本向量的值与s权重向量的点积
	{
		double ret = 0;
		while(x->index != -1)
		{
	...
	static void axpy(const double a, const feature_node *x, double *y)    //计算一个样本向量的值与常数a乘积的向量y
	{
		while(x->index != -1)
		{
...
class l2r_lr_fun: public function    //继承自function类，实现了L2正则化logistic回归的损失函数
{
public:
	l2r_lr_fun(const problem *prob, double *C);
	~l2r_lr_fun();

	double fun(double *w);                  //该类的损失函数值实现
	void grad(double *w, double *g);        //该类的梯度值计算实现
	void Hv(double *s, double *Hs);         //该类的Hessian矩阵与向量乘积的实现

	int get_nr_variable(void);              //该类获得特征向量的维数

private:
	void Xv(double *v, double *Xv);         //计算wTxi
	void XTv(double *v, double *XTv);

	double *C;                //惩罚因子
	double *z;                //用于存储wTxi的临时变量
	double *D;                //历史变量，存储logistic函数的导数，用于计算Hessian矩阵
	const problem *prob;      //训练样本集，计算损失函数、梯度要用到整个训练集
};

l2r_lr_fun::l2r_lr_fun(const problem *prob, double *C)    //该类的构造函数实现初始化 
{
	int l=prob->l;          //样本训练个数

	this->prob = prob;      //使该类的私有参数等于传入的参数（由于这两个参数同名，需要使用this）

	z = new double[l];      //分配z的存储空间，用于存储wTxi的临时变量
	D = new double[l];      //分配D的存储空间，存储logistic函数的导数
	this->C = C;            //初始化惩罚因子C
}

l2r_lr_fun::~l2r_lr_fun()
{
	delete[] z;              //释放两个在初始化过程中分配空间的指针
	delete[] D;
}


double l2r_lr_fun::fun(double *w)    //计算最优化目标的值，传入权重向量，返回函数的值
{
	int i;
	double f=0;                         //损失函数的值，累加每个样本的损失
	double *y=prob->y;                  //类别标签
	int l=prob->l;                      //样本数
	int w_size=get_nr_variable();       //权重向量的维数

	Xv(w, z);                           //计算每个wTxi，存在z中
...
/*
以上计算流程如下：
1.首先计算z=xw，其中为训练样本的特征集合矩阵，每一行为一个特征向量（一个样本），w为要求解的权重向量，计算的到的结果向量存储在中z。
2.计算正L2则化项1/2 wTw。
3.循环累加每个样本的损失函数，如果是正样本，则为，如果是负样本，则为。
*/

void l2r_lr_fun::grad(double *w, double *g)  //用于计算损失函数的梯度，其中g为返回的梯度向量
{
	int i;
	double *y=prob->y;                        //样本的类别标签
	int l=prob->l;                            //样本数
	int w_size=get_nr_variable();             //特征向量的维数

	//循环对每个样本计算
	for(i=0;i<l;i++)                 
	{
		z[i] = 1/(1 + exp(-y[i]*z[i]));
		D[i] = z[i]*(1-z[i]);             //D中存放logistic函数的导数
		z[i] = C[i]*(z[i]-1)*y[i];
	}
	XTv(z, g);                            //累加每个样本的z，即xTz 
...
void l2r_lr_fun::Hv(double *s, double *Hs) //计算Hessian矩阵与向量的乘积
{
	int i;
	int l=prob->l;
...
void l2r_lr_fun::Xv(double *v, double *Xv)  //用于计算wTxi
{
	int i;
	int l=prob->l;
	feature_node **x=prob->x;
...
void l2r_lr_fun::XTv(double *v, double *XTv)  //用于计算xTv
{
	int i;
	int l=prob->l;
	int w_size=get_nr_variable();
...
class l2r_l2_svc_fun: public function           //L2正则化L2损失函数 支持向量机 的损失函数
{
public:
	l2r_l2_svc_fun(const problem *prob, double *C);
	~l2r_l2_svc_fun();

	double fun(double *w);                     //计算函数值
	void grad(double *w, double *g);           //计算梯度值
	void Hv(double *s, double *Hs);            //计算Hessian矩阵

	int get_nr_variable(void);                 //返回特征向量维数

protected:
	void Xv(double *v, double *Xv);             //内部辅助函数，用于计算矩阵和向量乘法
...
double l2r_l2_svc_fun::fun(double *w)
{
	...
	for(i=0;i<w_size;i++)     //计算L2正则化项部分1/2wTw
		f += w[i]*w[i];
	f /= 2.0;
	for(i=0;i<l;i++)          //计算样本的L2损失值
	{
		z[i] = y[i]*z[i];
		double d = 1-z[i];
		if (d > 0)
...
void l2r_l2_svc_fun::grad(double *w, double *g)    //计算梯度值
{
	int i;
	double *y=prob->y;
	int l=prob->l;
...
void l2r_l2_svc_fun::Hv(double *s, double *Hs)      //计算Hessian矩阵
{
	int i;
	int w_size=get_nr_variable();
	feature_node **x=prob->x;
...
class l2r_l2_svr_fun: public l2r_l2_svc_fun     //L2正则化L2损失函数 支持向量回归 的目标函数
{
public:
	l2r_l2_svr_fun(const problem *prob, double *C, double p);

	double fun(double *w);               //计算目标函数值
	void grad(double *w, double *g);     //计算目标函数梯度
...
double l2r_l2_svr_fun::fun(double *w)      //计算目标函数值
{
	int i;
	double f=0;
	double *y=prob->y;
...
void l2r_l2_svr_fun::grad(double *w, double *g)    //计算目标函数梯度值
{
	int i;
	double *y=prob->y;
	int l=prob->l;
...
// A coordinate descent algorithm for                                     坐标下降算法
...
#define GETI(i) ((int) prob->y[i])                    //获取第i个样本的标签值
// To support weights for instances, use GETI(i) (i)

class Solver_MCSVM_CS                             //实现了求解多类SVM分类对偶问题的坐标下降法
{
	public:
		Solver_MCSVM_CS(const problem *prob, int nr_class, double *C, double eps=0.1, int max_iter=100000);
		~Solver_MCSVM_CS();
		void Solve(double *w);                    //求解接口函数
	private:
		//求解子问题
		void solve_sub_problem(double A_i, int yi, double C_yi, int active_i, double *alpha_new);
		//判断样本是否要消减掉
		bool be_shrunk(int i, int m, int yi, double alpha_i, double minG);
		double *B, *C, *G;       //B和C就是公式中的B，C
		int w_size, l;
		int nr_class;            //类型数，公式中的k
		int max_iter;            //最大迭代次数
		double eps;
		const problem *prob;
};
...
bool Solver_MCSVM_CS::be_shrunk(int i, int m, int yi, double alpha_i, double minG)    //判断样本是否要消减掉
{
	double bound = 0;
	if(m == yi)
		bound = C[GETI(i)];
...
void Solver_MCSVM_CS::Solve(double *w)    //求解整个问题的入口函数
{
	int i, m, s;
	int iter = 0;
...
static void solve_l2r_l1l2_svc(                      //实现求解L2正则化L1loss和L2loss支持向量机对偶问题的坐标下降法
	...
	double Cp, double Cn, int solver_type)
{
	...
	while (iter < max_iter)
	{
		...
		for (i=0; i<active_size; i++)     //先对样本进行随机洗牌
		{
			int j = i+rand()%(active_size-i);
			swap(index[i], index[j]);
		}

		for (s=0; s<active_size; s++)      //依次优化每一变量
		{
			i = index[s];                   //对第s个变量优化
			const schar yi = y[i];
			feature_node * const xi = prob->x[i];
	...
	double v = 0;                    //计算目标函数值，首先计算正则化部分
	int nSV = 0;
	for(i=0; i<w_size; i++)
		v += w[i]*w[i];
	for(i=0; i<l; i++)                 //然后计算损失函数部分
	{
...
static void solve_l2r_l1l2_svr(                               //求解L1loss和L2 loss epsilon-SVR（回归）对偶问题的坐标下降法
...
static void solve_l1r_l2_svc(                           //求解L1正则化L2损失函数支持向量机原问题的坐标下降法
...
static void solve_l1r_lr(                                   //求解L1正则化logistic回归原问题的坐标下降法
...
// transpose matrix X from row format to column format              函数transpose实现对矩阵X的转置操作
...
//训练一个分类或者回归模型，prob为训练样本数据，param为训练参数，w为训练得到的权重向量，Cp和Cn分别为正负样本的惩罚因子
...
//
// Interface functions   训练接口函数，传入参数为训练样本集数据prob，以及训练参数param，返回训练得到的模型
//
...
//predict_values为预测函数，对于输入特征向量x，计算预测值
...
//函数predict_probability是predict的加强版，不仅输出预测值，还返回概率估计值
