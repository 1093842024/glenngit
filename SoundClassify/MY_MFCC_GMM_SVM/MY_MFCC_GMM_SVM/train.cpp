...
static char* readline_train(FILE *input)   //对于输入的文件读每行的数据
{
	int len;
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name); //解析一组字符串命令，初始化求解参数
void read_problem(const char *filename);                              //将数据文本数据读入
void do_cross_validation();                                           //进行交叉验证
void do_find_parameter_C();                                           //找到参数C
...
int train_liblinear(int argc, char **argv)
{
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name)
{
	...
	for(i=1;i<argc;i++)
	{
		...
		{
			case 's':
				param.solver_type = atoi(argv[i]);    //atoi() 把字符串转换成整型数的函数
				flag_solver_specified = 1;
				break;

			case 'c':
				param.C = atof(argv[i]);             //atof()  把字符串转换成浮点数的函数
				flag_C_specified = 1;
				break;
