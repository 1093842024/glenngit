...
KMeans::KMeans(int dimNum, int clusterNum)
{
	...
	m_endError = 0.000001;      //所有类重心变化的误差大小
}
...
void KMeans::Cluster(double *data, int N, int *Label)
{
	...
	assert(size >= m_clusterNum); //特征向量样本的数量必须大于等于聚类中心的数量，因为至少一个聚类中心对应一个样本

	// Initialize model初始化聚类模型 解决聚类中每一类初始的均值m_means[m_clusterNum][m_dimNum]
	Init(data,N);
	...
	int unchanged = 0;
	bool loop = true;
	int* counts = new int[m_clusterNum];   //聚类的类数
	double** next_means = new double*[m_clusterNum];// New model for reestimation 重估的新聚类模型 对应m_means的下一次值
	for (int i = 0; i < m_clusterNum; i++)
	{
		next_means[i] = new double[m_dimNum];
	}
	...
	while (loop)
	{
		...
		// Classification
		for (int i = 0; i < size; i++)     //将所有的样本与kmean的类进行距离比较，以距离最近的类作为每个样本的标签，并计算所有的距离值，用于与上一次迭代对比
		{
			...
			counts[label]++;
			for (int d = 0; d < m_dimNum; d++)    //将每类的值累加到next_means中，用于后面的聚类均值更新
			{
				next_means[label][d] += x[d];
			}
		}
		...
		for (int i = 0; i < m_clusterNum; i++)
		{
			if (counts[i] > 0)
			{
				...
				memcpy(m_means[i], next_means[i], sizeof(double) * m_dimNum);//将计算的每类均值存到m_means中去
			}
		}
		...
		if (iterNum >= m_maxIterNum || unchanged >= 3)
		{
			if(iterNum >= m_maxIterNum ) printf("clustering reach max iternum!\n");
			loop = false;
		}

		//DEBUG
		printf("Cluster Iter:%1.0f , Average Cost: %f \n",iterNum,currCost);
		//cout << "Cluster Iter: " << iterNum << ", Average Cost: " << currCost << endl;
	}
...
void KMeans::Init(double *data, int N)   //传输指向数据的指针（数量大小*每个数据维度） 和 数据样本的数量大小
{
	int size = N;
	...
	if (m_initMode ==  InitRandom)
	{
		...
		double* sample = new double[m_dimNum];    //创建一个数据量的参数
	...
	else if (m_initMode == InitUniform)
	{
		...
		for (int i = 0; i < m_clusterNum; i++)
		{
			int select = i * size / m_clusterNum;       //等间距的选取某个数据的样本
			for(int j = 0; j < m_dimNum; j++)
				sample[j] = data[select*m_dimNum+j];    //将选取的该样本各个维度数据存入sample
			memcpy(m_means[i], sample, sizeof(double) * m_dimNum);  //将该样本数据复制到m_means中，作为该聚类的均值
		}
...
double KMeans::GetLabel(const double* sample, int* label)//将该样本点与所有类对比，将其加入距离最近的那一类，并返回类号label和距离值dist
{
	double dist = -1;
	for (int i = 0; i < m_clusterNum; i++)   
	{
