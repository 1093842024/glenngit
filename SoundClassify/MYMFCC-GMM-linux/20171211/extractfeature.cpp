...
double *extracttrain_mfcc(string wavpath,const int wavnum,long int &datasize, int &dim,string &feature)
{
	...
	{
		//clockBegin = clock();
		snprintf(wav_name,sizeof(wav_name), "%03d.wav", wav_idx);   string full_wav_name=wav_path+wav_name;
...
double *extracttrain_mfcc_diff_valide(string wavpath,const int wavnum,long int &datasize, int &dim,string &feature)
{
	dim=MFCCCOEF*2;
	feature="mfcc_diff";
	string wav_path=wavpath;
...
void testwavs(string wavpath,const int wavnum,GMM *gmm,const int dim,const string feature)
{
	...
	for(;wav_idx_test!=wav_num_test+1;wav_idx_test++)
	{
		...
		clockBegin = clock();
#endif
		snprintf(wav_name_test,sizeof(wav_name_test), "%03d.wav", wav_idx_test);   string full_wav_name_test=wav_path_test+wav_name_test;
...
if(proball>-20 &&probmax>-21) fitnum1++;//-22  -21    89%  15%   -17 -16  89%  15%   -17.5 -16.5    94%   7%   -18 -17       78%   15%   -21 -20   85%  8%                  
if(proball>-19 &&probmax>-20) fitnum2++;//-21  -20    78%  5%    -16 -15  86%   5%   -17    -16     86%   2%   -17.5 -16.5   72%  7%     -20 -19   79%  2%                           
if(proball>-18 &&probmax>-19)fitnum3++; //-20  -19    69%  2%    -15 -14  74%   2%   -16.5  -15.5   75%   1%   -17 -16       63%   4%    -19 -18   71%  0%                     
...
void testwavs_valide(string wavpath,const int wavnum,GMM *gmm,const int dim,const string feature)
{
	...
	for(;wav_idx_test!=wav_num_test+1;wav_idx_test++)
	{
		...
		probmax=log10(probmax);proball=log10(proball*FRAMENUM/validenum);
		printf("wave:%d  row:%d  maxprobability: %.3lf,allprob: %.2lf   \n",wav_idx_test,w->nRow,probmax,proball);
